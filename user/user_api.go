...
func (s *UserServer) GetUser(ctx context.Context, req *pb.GetUserRequest) (*pb.GetUserReply, error) {
	log.Printf("GetUser() - received: %v", req.GetName())
	id := db.UserIDFromResourceName(req.GetName())
	u, err := db.ReadUser(id)
	if err != nil {
		return nil, err
	}
...
func (s *UserServer) AuthenticateUser(ctx context.Context, req *pb.AuthenticateUserRequest) (*pb.AuthenticateUserReply, error) {
	...
	userId, err := db.AuthenticateUser(req.GetUsername(), req.GetPassword())
	if err != nil {
		return nil, err
	}
	log.Printf("AuthenticateUser() - returning: %s", userId)
